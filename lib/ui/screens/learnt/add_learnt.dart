...
class AddLearnt extends StatefulWidget {
  AddLearnt({this.email});
  final String email;

  @override
  State createState() => _AddLearntState();
}

class _AddLearntState extends State<AddLearnt> {
  StateModel appState;
  ...
  Future _addLearnt({Learnt learnt}) async {
    //verifying userType is Direct or Franchise to determine category

    learnt.date=DateTime.now().year.toString()+'/'+DateTime.now().month.toString()+'/'+DateTime.now().day.toString();
    ...
    if (_formKey.currentState.validate()) {
      try {
        ...
        var id = utf8.encode(learnt.description +
            DateTime.now().millisecondsSinceEpoch.toString()); // data being hashed


        learnt.id = sha1.convert(id).toString();


        MyService.addLearnt(learnt).then((onValue) {
          if (onValue) {
            AlertDiag.showAlertDialog(context, 'Status',
                'Idea Successfully Added', MyRoutes.VIEW_LEARNT);
          } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (!appState.isLoading &&
      ...
      final questionField = Visibility(
        visible: _categoryVisibility,
        child: TextFormField(
            autofocus: false,
            keyboardType: TextInputType.text,
        ...
        child: RaisedButton(
            ...
            onPressed: () {
              setState(() {
                ...
                Learnt learnt=new Learnt();
                learnt.userId = userId;
                learnt.description = descriptionController.text;
                learnt.question=questionController.text;

                _addLearnt(learnt: learnt);
              });
            }),
      );
      ...
      return WillPopScope(
          ...
          child: Scaffold(
              appBar: new AppBar(
                ...
                title: Text('Add What You Learnt'),
              ),
              body: LoadingScreen(
                child: form,
